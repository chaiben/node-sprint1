...
const decriptyData = (encryeted) => {
  ...
  const decipher = crypto.createDecipheriv(ALGORITME, key, iv)
...
const descriptFile = () => {
  fs.readFile('FileHexCrypted.txt', 'utf-8', (_err, criptyData) => {
    const data = decriptyData(criptyData)
  ...
  fs.readFile('File64Crypted.txt', 'utf-8', (_err, criptyData) => {
    const data = decriptyData(criptyData)
    // let buff = Buffer.from(data);
